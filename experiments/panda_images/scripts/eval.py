...
from custom_andp import CNN_weight, cnn_ANDP
...
graphics.record_video(EXPERIMENT_NICE_NAME + "_" +
                      TARGET_MOTION+"-env-eval.mp4", simu.graphics_freq())
...
if EXPERIMENT == 'ANDPS':
    num_DS = 8
    net = cnn_ANDP(3, num_DS, torch.Tensor(target),
                   CNN_weight(3, num_DS), device)
    net.load_state_dict(torch.load(
        "models/andps_images_100.pt", map_location=device))
elif EXPERIMENT == 'CNN':
    net = simple_cnn(ds_dim=3)
    net.load_state_dict(torch.load(
        "models/simple_cnn_100.pt", map_location=device))
    ...
    if simu.scheduler().schedule(control_freq):
        ...
        if (APPLY_PERTURBATION):
            ...
            if (PERTURBATION_TYPE == "push"):
                if (0 <= t and t <= 0.2):
                    robot.set_external_force(
                        eef_link_name, [-50., -100., -80.])
                elif (4 <= t and t <= 4.3):
            ...
            box_packages = [
                (TARGET_MOTION_TWO+"_box", "urdfs/"+TARGET_MOTION_TWO+"_box")]
