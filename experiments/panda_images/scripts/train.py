...
from andps import CustomDataset
from custom_andp import cnn_ANDP, CNN_weight
...
for i, datum in enumerate(data):
    if (i == 0):
        np_X = torch.Tensor(datum["np_X"])
        np_Y = torch.Tensor(datum["eef_vx"])
        target = datum["eef_x"][-1, :]
    else:
        cur_np_X = torch.Tensor(datum["np_X"])
        np_X = torch.cat((np_X, cur_np_X), 0)
        np_Y = torch.cat((np_Y, torch.Tensor(datum["eef_vx"])), 0)
...
if EXPERIMENT == 'ANDPS':
    ...
    num_DS = 8
    net = cnn_ANDP(dim, num_DS, torch.Tensor(target),
                   CNN_weight(dim, num_DS), device=device)
