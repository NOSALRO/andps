...
from utils import net_first_order as net_a, decorate_axis
...
for id in range(0, len(names)):
    ...
    w_ymin = w_y[0]
    w_ymax = w_y[1]
    data_lasa = np.load(os.path.join(lasa_dataset_folder, name + '_' + str(k) + '.npz'))
    ...
    net.load_state_dict(torch.load('models/lasa_' +name + '_'+str(k) + '_1000.pt', map_location='cpu'))
    net.eval()
    ...
    for idk in range(0, len(start_idxs)):
        ...
        for _ in range(2000):
            # ,False)[0].detach().cpu()
            v = net(torch.Tensor(
                (np.array([x_cur_0[0] , x_cur_0[1] ]))).view(-1, 2), False)[0].detach().cpu()
    ...
    for idk in range(0, len(start_idxs)):
        ...
        # start Evaluation
        for _ in range(2000):
            v = net(torch.Tensor((np.array([x_cur_0[0] , x_cur_0[1]]))).view(-1, 2), False)[0].detach().cpu()
            x_cur_0[0] = x_cur_0[0] + dt_global * v[0, 0].numpy()
            x_cur_0[1] = x_cur_0[1] + dt_global * v[0, 1].numpy()
            if (x_cur_0[0] > w_xmax):
                break
