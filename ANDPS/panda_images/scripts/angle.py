...
# box
angle_box_packages = [("angle_box", "urdfs/angle_box")]
angle_box = rd.Robot("urdfs/angle_box/angle_box.urdf",  angle_box_packages,"angle_box")
angle_box.set_color_mode("material")
angle_box.fix_to_world()
box_tf = angle_box.base_pose()
box_tf.set_translation([3.5, 0., 1.])
angle_box.set_base_pose(box_tf)
simu.add_robot(angle_box)
...
graphics.camera().record(True)
graphics.record_video("angle-env.mp4", simu.graphics_freq())
...
camera.record_video("angle-pov.mp4")
...
plt.show()
np.savez('data/angle.npz', eef_x =eef_x, eef_vx=eef_vx, images = images[:,:,1:])
