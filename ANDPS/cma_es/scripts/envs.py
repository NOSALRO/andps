...
class PourEnv(gym.Env):
    ...
    def __init__(self, enable_graphics=False, enable_record=False, seed=-1, dt=0.01):
        ...
        # define action space
        self.action_space = gym.spaces.Box(low=np.array([-1, -1, -1, -1, -1, -1]), high=np.array([1, 1, 1, 1, 1, 1]), shape=(6,), dtype=np.float32)
        self.low_bounds = [self.table.base_pose().translation()[0]-1.5, self.table.base_pose().translation()[1]-1., self.table.base_pose().translation()[2]+0.35]
        self.high_bounds = [self.table.base_pose().translation()[0]+1.5, self.table.base_pose().translation()[1]+1., self.robot.body_pose("panda_ee").translation()[2]+1.]
